...
x_image = tf.placeholder("float", [None, 28, 28, 28, 28, 1])
...
layer1, layer1_weight, layer1_bias = new_conv_nd_layer(input=x_image, filter_size=[5, 5, 5, 5], num_filters=8,
                                                       pooling_type='max', pooling_strides=[1, 2, 2, 2, 2, 1],
                                                       pooling_ksize=[1, 2, 2, 2, 2, 1], pooling_padding='VAILD',
                                                       strides=[1, 1, 1, 1, 1, 1], padding='SAME',
                                                       method='convolution', weights=W_conv1, biases=b_conv1)
...
layer2, layer2_weight, layer2_bias = new_conv_nd_layer(input=x_image, filter_size=[5, 5, 5, 5], num_filters=16,
                                                       ...
                                                       pooling_ksize=[1, 2, 2, 2, 2, 1], pooling_padding='VAILD',
                                                       strides=[1, 1, 1, 1, 1, 1], padding='SAME',
                                                       method='convolution', weights=W_conv2, biases=b_conv2)
...
images = [np.zeros(21952), np.ones(21952)]
labels = [[1, 0], [0, 1]]

for i in range(100):
    train_step.run(feed_dict={x_image: images, y_: labels, keep_prob: 1})

print("test accuracy %g" % accuracy.eval(feed_dict={
    x_image: images, y_: labels, keep_prob: 1.0}))
